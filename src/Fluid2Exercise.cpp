...
void Fluid2::initParticles()
{
    ...
    for (int i = 0; i < Scene::nCellsX; i++) {
        for (int j = 0; j < Scene::nCellsY; j++) {
            ...
            posX = cellPos.x + rX * subcell_width;
            posY = cellPos.y + rY * subcell_height;
            this->particles.addParticle(Vector2(posX, posY), vel, ink);

            // subcelda 10
            rX = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
            rY = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
            posX = cellPos.x + subcell_width + rX * subcell_width;
            posY = cellPos.y + rY * subcell_height;
            this->particles.addParticle(Vector2(posX, posY), vel, ink);
            ...
            rY = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
            posX = cellPos.x + rX * subcell_width;
            posY = cellPos.y + subcell_height + rY * subcell_height;
            this->particles.addParticle(Vector2(posX, posY), vel, ink);
        ...
        Array2<float> velYCopy = this->velocityY;
        {
            ...
            for (uint i = 0; i < Scene::nCellsX; i++) {
                for (uint j = 0; j < Scene::nCellsY; j++) {
                    ...
                    // se obtiene el vector velocidad en el punto medio. para ello se interpola el de cada cara de la
                    // rejilla
                    float v_x = (velXCopy[Index2(i, j)] + velXCopy[Index2(i + 1, j)]) * 0.5f;
                    float v_y = (velYCopy[Index2(i, j)] + velYCopy[Index2(i, j + 1)]) * 0.5f;
                    Vector2 v(v_x, v_y);

                    // ecuacion de cinematica para interpolacion de posiciones, con la velocidad invertida para saber el
                    // paso anterior
...

        {
            ...
            for (uint i = 0; i < facesX_size.x; i++) {
                for (uint j = 0; j < facesX_size.y; j++) {
                    ...
                    float vy_center = (i == facesX_size.x - 1)
                                          ? 0.f
                                          : velYCopy[Index2(i, j)];  // vy tiene facesX_size.x - 1 elementos
...
void Fluid2::fluidViscosity(const float dt)
{
    ...
    // se resuelve la formula del termino viscoso de navier-strokes con una aproximacion explicita por diferencias
    // finitas v' = v + (dt / densidad) * viscosidad * ((v_right - 2*v + v_left) / dx^2 + (v_up - 2*v + v_down) / dy^2)
...
void Fluid2::fluidPressureProjection(const float dt)
{
    ...
    for (int i = 0; i < Nx; i++) {
        for (int j = 0; j < Ny; j++) {
            ...
            // Si aislamos los valores de p de la ecuación, nos queda una matriz que hay que rellenar de la siguiente
            // manera. vecino arriba = -1 (si existe) vecino abajo = -1 (si existe) vecino dcha = -1 (si existe) vecino
            // izda = -1 (si existe) elemento central = nº de vecinos use add_to_element, set_element
    ...
    // residual_out: mide la precisión del resultado. si la función devuelve true, significa que residual_out tiene un
    // valor menor que una tolerancia establecida iterations_out: nº de iteraciones realizadas para alcanzar el
    // resultado.
    ...
    // Se debe calcular la media de las presiones, para asegurar que las presiones estén siempre centradas en 0 y
